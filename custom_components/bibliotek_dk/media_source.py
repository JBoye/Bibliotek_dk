...
class BibliotekMediaSource(MediaSource):
    ...
    async def async_browse_media(self, item: MediaSourceItem) -> BrowseMediaSource:
        ...
        if "|" in identifier:
            entry_id, order_id = identifier.split("|", 1)
        ...
        children = [
            BrowseMediaSource(
                domain=self.domain,
                identifier=f"{entry_id}|{book.get("order_id")}",
    ...
    async def async_resolve_media(self, item: MediaSourceItem) -> PlayMedia:
        ...
        try:
            entry_id, order_id = item.identifier.split("|", 1)
        except ValueError:
        ...
        loans = library.get_audiobooks()
        book = next(
            (b for b in loans if b.get("order_id") == order_id),
            None,
        )

        if not book:
            raise ValueError(f"Book not found: {order_id}")
        ...
        if not os.path.isfile(output_path):
            ...
            try:
                _LOGGER.info("Downloading audiobook to: %s", output_path)
